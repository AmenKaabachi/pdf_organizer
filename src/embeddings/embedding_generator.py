...
# Be defensive: importing sentence-transformers can pull in large dependencies
# (transformers, tokenizers) which may raise MemoryError or other runtime errors
# on constrained environments. Catch any Exception and disable local models
# rather than letting import-time errors crash test runs or the CLI.
try:
    ...
    logger.warning(
        "⚠️ sentence-transformers (or its dependencies) could not be imported. "
        "Local models will be disabled. Reason: %s", str(e)
    )
...
class EmbeddingGenerator:
    ...
    def __init__(self, model_name: str = 'all-MiniLM-L6-v2', 
        ...
        if self.uses_local:
            # LOCAL MODEL SETUP (defer heavy model loading until actually needed)
            ...
            # Defer actual model loading until embeddings are requested to avoid
            # expensive downloads or memory spikes at import/startup time.
            self.model = None
    ...
    def _load_local_model(self):
        ...
        # Try loading the requested local model. If it fails (MemoryError, OSError,
        # or other exceptions due to environment constraints), try a sequence of
        # progressively smaller multilingual models as fallbacks to avoid crashing
        # the process on low-memory machines.
        fallback_candidates = [
            self.model_name,
            # Recommended smaller multilingual fallbacks (quality -> size)
            'paraphrase-multilingual-mpnet-base-v2',  # ~1.11GB
            'sentence-transformers/distiluse-base-multilingual-cased-v2',  # ~540MB
        ...
        for candidate in fallback_candidates:
            try:
                logger.info(f"Attempting to load local model: {candidate}")
                # instantiate model
                model = SentenceTransformer(candidate, device=self.device)
                self.model = model
                self.model_name = candidate
                self.embedding_size = model.get_sentence_embedding_dimension()
                logger.info(f"✅ Local model loaded successfully: {candidate} (embedding size: {self.embedding_size})")
                return
            except Exception as e:
                last_exception = e
                logger.warning(f"Failed to load local model '{candidate}': {e}")
                # Try next candidate

        # If none of the candidates could be loaded, raise the last exception
        logger.error(f"❌ Could not load any local model from candidates: {fallback_candidates}")
    ...
    def _generate_local_embeddings(self, texts: List[str], batch_size: int, show_progress: bool) -> np.ndarray:
        ...
        try:
            ...
            # Ensure model is loaded (deferred load)
            if not hasattr(self, 'model') or self.model is None:
                logger.info("Local model not yet loaded — attempting to load now")
    ...
    def generate_document_embeddings(self, extraction_results: List[Dict], 
        ...
        for result in extraction_results:
            ...
            if isinstance(result, dict):
                ...
                # Limit or chunk text length - local models can handle more text
                words = text.split()
                if self.uses_local:
                    # Local models can handle larger chunks (use 2000 words per chunk)
                    max_words = 2000
                else:
                    # API models should use less to avoid costs (use 1500 words per chunk)
                    max_words = 1500

                # If the document is longer than max_words, split into overlapping chunks
                # (sliding window) so we better capture context across page boundaries.
                if len(words) > max_words:
                    # Use 50% overlap by default
                    stride = max(1, int(max_words * 0.5))
                    chunks = []
                    ...
                    # Store chunked text joined with a special marker per-document so we can
                    # remember how many chunks belong to each document when aggregating.
                    # We'll append the chunks to the global texts list and keep a mapping.
                    if not hasattr(self, '_doc_chunk_map'):
                        self._doc_chunk_map = []

                    start_idx = len(texts)
                    texts.extend(chunks)
                    end_idx = len(texts)
                    # record how many chunks belong to this document
                    self._doc_chunk_map.append((filename, start_idx, end_idx))
                    document_names.append(filename)
                else:
                    # Short document: keep as single chunk
                    texts.append(text)
                    # For consistency record in chunk map with same start/end
                    if not hasattr(self, '_doc_chunk_map'):
                        self._doc_chunk_map = []
                    idx = len(texts) - 1
                    self._doc_chunk_map.append((filename, idx, idx + 1))
                    document_names.append(filename)
        ...
        # Generate embeddings for all (possibly chunked) texts
        embeddings = self.generate_embeddings(texts, batch_size, show_progress)

        # If chunking was used, aggregate chunk embeddings per document by averaging
        if hasattr(self, '_doc_chunk_map') and self._doc_chunk_map:
            doc_embeddings = []
            doc_names_ordered = []
            for (fname, start_idx, end_idx) in self._doc_chunk_map:
                # Slice embeddings for this document's chunks
                chunk_embs = embeddings[start_idx:end_idx]
                if chunk_embs.size == 0:
                    continue
                # Average along axis 0
                avg_emb = np.mean(chunk_embs, axis=0)
                # Re-normalize averaged embedding to unit length if possible
                norm = np.linalg.norm(avg_emb)
                if norm > 0:
                    avg_emb = avg_emb / norm
                doc_embeddings.append(avg_emb)
                doc_names_ordered.append(fname)

            embeddings = np.vstack(doc_embeddings)
            # Clear temporary chunk map for next call
            self._doc_chunk_map = []
            document_names = doc_names_ordered
