...
# Heavy imports (PDF extraction, embeddings, clustering) are imported lazily
# inside functions to avoid importing torch/sentence-transformers at module
# import time which can confuse Streamlit's file watcher (and trigger
# issues on some environments). See process_documents() for local imports.
...
def main():
    ...
    with st.sidebar:
        ...
        # Use all-mpnet-base-v2 for clustering stability and quality
        SELECTED_MODEL = 'all-mpnet-base-v2'
        st.write(f"Using single model for clustering: **{SELECTED_MODEL}**")
        selected_model = SELECTED_MODEL
        st.write("**Model:** all-mpnet-base-v2 — high-quality sentence embeddings (recommended for clustering).")
        st.write("**Note:** Local inference requires 'sentence-transformers' and 'torch' and sufficient RAM/disk space.")
        ...
        if clustering_algorithm == "kmeans":
            # Enforce auto-tune (model chooses number of clusters)
            auto_tune = True
            n_clusters = st.slider(
                "Number of Clusters",
                2, 10, 3,
                disabled=True,
                help="Auto-detect is enforced; the model will select the number of clusters"
            )
            st.info("🤖 Auto-tune enforced: Optimal cluster count will be determined automatically")
...
def process_documents(uploaded_files, model_name, algorithm, n_clusters, auto_tune, eps, min_samples):
    ...
    # Local imports to avoid heavy ML libs (torch, sentence-transformers) at
    # module import time which can confuse Streamlit's watcher on some systems.
    ...
    try:
        ...
        # Disable embedding cache to force fresh embeddings for naming
        embedding_generator = EmbeddingGenerator(model_name=model_name, cache_embeddings=False)
...
def show_analysis_results():
    ...
    # Use container width to allow Streamlit to size the table responsively
